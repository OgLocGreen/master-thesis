...
for i in range(len(grouped_contours)):
    ...
    # If border points were found, add them to a new hull
    if len(border_points) > 0:
        border_hull = cv2.convexHull(np.array(border_points))
        hull = np.vstack((hull, border_hull))
    
    # Draw the hull on the mask
    cv2.drawContours(mask, [hull], 0, (0, 0, 255), 2)
    cv2.drawContours(mask_only_hull, [hull], 0, (0, 0, 255), 24)

# Save the result image
cv2.imwrite("closed_contours