...
def generate_line_picture(angle):
    """
    Generate a line picture with the maximum length at the specified angle.
    ...
    max_length = max(plt.rcParams['figure.figsize'])  # Maximum length based on figure size

    x = np.linspace(0, max_length * np.cos(angle_rad), num=2)
    y = np.linspace(0, max_length * np.sin(angle_rad), num=2)

    plt.plot(x, y)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title('Line Picture')
    plt.grid(True)
    plt.axis("equal")
    plt.show()
...
def find_line_intersections_trap(line_start, line_end, trapezoid):
    """
    Find the intersections between a line and a trapezoid.
...
if __name__ == '__main__':
    ...
    # erstellen von einer line je nach Winel angabe.
    # Winkel sollte immer zwischen 0 - 179 sein da sonst die linen die gleichen sind.
    # Generell könnte man auch 0-360 machen aber macht ja kein unterschied

    # Problem ist wo soll der erste Path beginne?
    # 1. Idee einfach ganze bild mit linen voll machen (Bild komplett mit abdecken) und immer bei 0,0 anfangen.
    # Problem hierbei bei Bsw. 45grad bei 0,0 befindet sich in der rechten ecken keine Linen.
    # Workaround???
    # 1.1 einfach in der mitte vom Bild anfangen und dann auf beide seiten Parrelel lien solange machen wie es geht
    # -> Problem linen sind nicht nacheinander array mit Pfadpunkten muss dannach noch mal sotiert werden
    # 1.2 Punkt finden und dann starten wie bei Polygon (Begin Seite) finden
    # Problem Konture könnte mehrer "minimas" haben
    # 1.3 einfach beim Polygone bleiben was vohher so ausgemacht. 

    # dannach kann einfach wie bekannt gemacht werden. 
    # 1. Line erstellen 
    # 2. Checken if e die Hülle schneidet
    # 3. if true: hinzufügen der Acceloration point und breaking point
    #       dann aufnehmmen der Punkte in die Pfadliste
    # else: pass
    # erstellen von pareller Line der ersten line
    # solange wieter achen wie möglich

    generate_line_picture(30)
