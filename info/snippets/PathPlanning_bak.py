...
def make_parallel_line(point1, point2, distance, direction, height = 500, width = 500):
    ...
    offset_vector = perpendicular_vector * distance
...
def calculate_path(polygon, img, width, height, distance_a, distance_b, angle, x_distance, y_distance):
    path_points = []
    ...
    #cv2.line(img, last_line[0], last_line[1], (0, 255, 0), 1)
    #cv2.imshow("img", img)
    #cv2.waitKey(0)
    ...
    if path_points == []:
        polygon_points = find_line_intersections(line[0], line[1], polygon)
        middle_points = interpolate_points(polygon_points[0],polygon_points[1], distance_b)
        path_points.append([middle_points[0], polygon_points[0], polygon_points[1],  middle_points[1]])
    ...
    for path in path_points:
        ...
        for point in path:
            x = point[0]
            y = point[1]
            angle_radians = np.deg2rad(180-angle)
            x_distance_angle = np.cos(angle_radians)*x_distance - np.sin(angle_radians)*y_distance
            y_distance_angle = np.sin(angle_radians)*x_distance + np.cos(angle_radians)*y_distance
            #angle_radians = math.radians(angle)
            #x_distance_angle = math.cos(angle_radians)*x_distance - math.sin(angle_radians)*y_distance 
            #y_distance_angle = math.sin(angle_radians)*x_distance + math.cos(angle_radians)*y_distance
...
def make_contours(img, threshold_detail_polygon):
    ...
    # function to extract first number from image filename
def extract_number(filename):
        # regular expression to find first number in filename
...
def corrdinates_to_pixel(x,y):
    y_pixel = 1654/14.9433
    x_pixel = 591/7.989820
    return (int(x*x_pixel), int(y*y_pixel))

def pixel_to_corrdinates(x,y):
    y_pixel = 14.9433/1654
    x_pixel = 7.989820/591
    return (int(x*x_pixel), int(y*y_pixel))
...
if __name__ == '__main__':
    ...
    image_folder = './snippets/images/pathplanning'

    # get a list of all image files in the directory
    image_files = [f for f in os.listdir(image_folder+"/input/") if f.endswith('.png')]



    # sort image files based on first number in filename
    ...
    for image_file in image_files:
        # open image file
        img = cv2.imread(image_folder+ "/input/" + image_file)
    


        save_path = image_folder + "/results/result_"+ image_file
        ...
        # Calculate the path
        distance_a = 15 # distance between the lines
        distance_b = 5  # distance between the points
        angle = extract_last_number(image_file)
        ...
        L_T = 10
        W_T = 10                        
        t_x = np.ceil(L_R/2) + L_T/2 
        t_y = np.ceil(W_R/2) + W_T/2 

        t_x_pixel, t_y_pixel = corrdinates_to_pixel(t_x/100, t_y/100)
        ...
        x_distance = t_x_pixel
        y_distance = t_y_pixel
    ...
    arrays_list = [[np.array(t) for t in sublist] for sublist in path_planing]


    #np.save(image_folder+"/results/"+'array_file.npy', path_planing, allow_pickle=True, fix_imports=True)
    np.savetxt(image_folder+"/results/"+'array_pathpoints.txt', arrays_list, fmt='%s')
    np.savetxt(image_folder+"/results/"+'array_file_orientation.txt', orientation_list, fmt='%s')
    #np.save(image_folder+"/results/"+'array_file_orientation.npy', orientation_list, allow_pickle=True, fix_imports=True)

    index_zone = 0
    next_point_index = 0
    x = 0
    test = path_planing[index_zone][next_point_index]
    test = path_planing[index_zone][next_point_index][0]
    if(abs(path_planing[index_zone][next_point_index][0]-x) <= 10) :
        pass

    test_1 = np.loadtxt(image_folder+"/results/"+'array_pathpoints.txt',)
    test_2 = np.loadtxt(image_folder+"/results/"+'array_file_orientation.txt')
