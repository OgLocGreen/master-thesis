...
# Draw the new convex hull on a black image
image_with_hull2 = np.zeros_like(image)
...
for hull in hull_goup:
    ...
    edges2 = edges
    border_left, border_right, border_top, border_bottom = False, False, False, False 
    ...
    if border_left and border_top:
        new_edge.append((0,0))
    elif border_left and border_bottom:
        new_edge.append((0,image.shape[0] - 1))
    elif border_right and border_top:
        new_edge.append((image.shape[1] - 1,0))
    elif border_right and border_bottom:
        new_edge.append((image.shape[1] - 1,image.shape[0] - 1))

    edges2.append(tuple(new_edge)) ### Why we have here a tuple of a tuple??
    ## edges2 there are like this ((77,590),(77,590))
    ## why dont we have just like (x,y) ??
    ## what is the second tuple for??
    ## top we loop froth the edges but why does one edge have two tuples??
    

    # Create a new convex hull from the updated edges
    hull_from_edges2 = cv2.convexHull(np.array(edges2).reshape(-1, 2), clockwise=False)
    cv2.drawContours(image_with_hull2, [hull_from_edges2], 0, 255, -1)
