...
from inspection_realsense.msg import moveBackwardAction, moveBackwardGoal, captureImagesAction, captureImagesGoal
...
class moveBackwardAndCapture(EventState):
	'''
	...
	def __init__(self, pathplanning_vel):
		...
		super(moveBackwardAndCapture, self).__init__(outcomes = ['done','failed'],input_keys=['n_pasadas_ant', 'n_imgs_ant', 'folder_path'],output_keys = ['n_pasadas_actuales', 'n_imgs_actuales'])
		...
		self._topic = 'moveBackward' #Aqui mismo topic que en el server
		self._client = ProxyActionClient({self._topic: moveBackwardAction}) # pass required clients as dict (topic: type)
	...
	def on_enter(self, userdata):
		...
		try:
			...
			Logger.logwarn('Failed to send the moveBackward command:\n%s' % str(e))
			self._error = True
