...
from irobot_inspection_pckg.msg import getCompletePanoramaAction, getCompletePanoramaGoal
...
class getCompletePanoramaState(EventState):
	'''
	...
	def __init__(self):
		...
		super(getCompletePanoramaState, self).__init__(outcomes = ['done','failed'], input_keys = ['name_0', 'name_180','folder_pano_path'])


		
		self._topic = 'getCompletePanorama' #Mismo nombre que en el server
		...
		self._client = ProxyActionClient({self._topic: getCompletePanoramaAction})
	...
	def on_enter(self, userdata):
		...
		# Create the goal.
		goal = getCompletePanoramaGoal()
		goal.name_0.data = userdata.name_0
		goal.folder_pano_path.data = userdata.folder_pano_path
		goal.name_180.data = userdata.name_180
		...
		try:
			...
			Logger.logwarn('Failed to send the getCompletePanorama command:\n%s' % str(e))
			self._error = True
