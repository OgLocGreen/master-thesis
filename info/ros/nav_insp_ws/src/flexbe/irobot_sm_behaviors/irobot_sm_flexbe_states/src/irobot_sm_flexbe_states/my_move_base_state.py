...
class MyMoveBaseState(EventState):
    """
    Navigates a robot to a desired position and orientation using move_base (modified example).
    ...
    def __init__(self):
        ...
        super(MyMoveBaseState, self).__init__(outcomes = ['arrived', 'failed'],
                                            input_keys = ['waypoint'])
    ...
    def on_enter(self, userdata):
        ...
        goal.target_pose.header.frame_id = "summit_xl_base_footprint" # #odom en el original
        # goal.target_pose.header.stamp.secs = 5.0

    	Logger.logwarn("%s" % str(goal.target_pose.pose))
