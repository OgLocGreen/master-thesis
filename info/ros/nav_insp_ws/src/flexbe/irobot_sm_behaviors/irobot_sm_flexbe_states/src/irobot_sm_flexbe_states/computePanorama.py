...
from inspection_realsense.msg import computePanoramaAction, computePanoramaGoal
...
class computePanoramaState(EventState):
	'''
	...
	def __init__(self):
		...
		super(computePanoramaState, self).__init__(outcomes = ['done','failed'], input_keys = ['folder_path', 'direction','folder_pano_path', 'step','width_sheet', 'long_sheet'])


		
		self._topic = 'computePanorama' #Mismo nombre que en el server
		...
		self._client = ProxyActionClient({self._topic: computePanoramaAction})
	...
	def on_enter(self, userdata):
		...
		try:
			...
			Logger.logwarn('Failed to send the computePanorama command:\n%s' % str(e))
			self._error = True
