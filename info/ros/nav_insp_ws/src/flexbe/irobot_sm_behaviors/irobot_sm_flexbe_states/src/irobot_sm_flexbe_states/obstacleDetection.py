...
from inspection_realsense.msg import obstacleDetectionAction, obstacleDetectionGoal
...
class obstacleDetection(EventState):
	'''
	...
	def __init__(self):
		...
		super(obstacleDetection, self).__init__(outcomes = ['done','failed', 'obstaculo'])
		...
		self._topic = 'obstacleDetection' #Aqui mismo topic que en el server
		self._client = ProxyActionClient({self._topic: obstacleDetectionAction}) # pass required clients as dict (topic: type)
	...
	def on_enter(self, userdata):
		...
		# Create the goal.
		goal = obstacleDetectionGoal()
		goal.find_obstacle = True
		...
		try:
			...
			Logger.logwarn('Failed to send the obstacleDetection command:\n%s' % str(e))
			self._error = True
