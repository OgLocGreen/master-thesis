...
from inspection_realsense.msg import computeDefectPolygonsAction, computeDefectPolygonsGoal
...
class computeDefectPolygonsState(EventState):
	'''
	...
	def __init__(self):
		...
		super(computeDefectPolygonsState, self).__init__(outcomes = ['done','failed'], input_keys = ['folder_pano_path','width_sheet', 'long_sheet'], output_keys=['polygons'])


		
		self._topic = 'computeDefectPolygons' #Mismo nombre que en el server
		...
		self._client = ProxyActionClient({self._topic: computeDefectPolygonsAction})
	...
	def execute(self, userdata):
		...
		if self._client.has_result(self._topic):
			...
			userdata.polygons = result.polygons
	...
	def on_enter(self, userdata):
		...
		# Create the goal.
		goal = computeDefectPolygonsGoal()
		...
		try:
			...
			Logger.logwarn('Failed to send the computeDefectPolygons command:\n%s' % str(e))
			self._error = True
