...
class ComputeDefectsDecompositionState(EventState):
	'''
	State that activate the decomposition of the defect structure.
	...
	def __init__(self):
		...
		super( ComputeDefectsDecompositionState, self).__init__(outcomes = ['done','failed'])
		...
		self._client = rospy.ServiceProxy('/irobot_ir_data', IROBOT_ir_data) # pass required clients as dict (topic: type)
	...
	def execute(self, userdata):
		...
		if self._client.has_result(self._topic):
			...
			##Adaptar la peticion de descomposicion de todos los defectos
	...
	def on_enter(self, userdata):
		...
		# Create the goal.
		goal = RectangleFCPPGoal()
		goal.rectangle_id = id_zone
		...
		try:
			...
			Logger.logwarn('Failed to send the ExecuteRectangleFCPP command:\n%s' % str(e))
			self._error = True
