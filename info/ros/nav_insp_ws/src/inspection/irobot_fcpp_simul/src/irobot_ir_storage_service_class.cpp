...
//    vec_trapeciosarray_vizmsg.reserve(10);
  ...
  //  reference_frame_string="world";
    reference_frame_string="sheet_inspection_frame";
...
void IROBOT_ir_Storage_Service_Class::computePolyDetection()
{
    ...
    // send a goal to the action
       ...
       // goal.path.data="/home/irobot/imgs_panorama/panoramaCompleto.png"; // NOMBRE QUE PASE COMO ENTRADA AL ESTADO
        goal.path.data = path_panorama;
...
//            }
            ...
            footprint_pub.publish(poly_footprint_vizmsg);
...
ROS_INFO("-----> Inicio de llamada al servicio de almacenamiento");
    ...
    if(req.load_inputdata)
    {
        ...
        this->id_defect_repair = req.id_defect_repair;
        this->path_panorama = req.path_panorama;
    ...
    {
        ROS_INFO("Respuesta con los datos de entrada");
    ...
    {
        ROS_INFO("Getting inspection zones");
    ...
    {
        ROS_INFO("Getting working zones H");
    ...
    {
        ROS_INFO("Getting working zones C");
    ...
    if(req.get_defect_i)
    {
        ...
        ROS_INFO("Numero defectos: %d",defectos_vec.size());

    }
//////
    if (req.get_inspection_path)
    {
        ROS_INFO("Getting inspection paths");
...
//        }
        ...
        res.defect_i_paths=preparePathForExecution(paths_defects_vector.at(/*defect_goal*/id_defect_repair));

        ROS_INFO("res devuelto");

        std::vector<geometry_msgs::PoseArray> aux_tool_path=preparePathForExecution(paths_tool_defects_vector.at(/*defect_goal*/id_defect_repair));

        ROS_INFO("preparePath listo");
...
//        ROS_INFO("%d", res.defect_i_paths.size());
    ...
    if(req.get_rot_zone)
    {
        ...
        ROS_INFO("Getting rotation zone");
    ...
    {
        ROS_INFO("Activating Rviz visualizations");
    ...
    {
        ROS_INFO("Compute path defect i");
    ...
    {
        ROS_INFO("Compute inspection path zone i");
    ...
    {
        ROS_INFO("Changing inspection goal");
    ...
    {
        ROS_INFO("Changing inspection path");
                                                                                         ...
                                                                                         double tras_act_x, double tras_act_y)
{
    ...
    //ESTO ESTÁ INTERCAMBIADO, PERO SI LO CAMBIO ROMPE
        ///Parametros chapa
        double hc=long_area_y*100*0.5; //*100 en esta funcion se trabaja en cm
        double lc=long_area_x*100*0.5;
        ...
        cv::Mat img=Mat(lc2_pix,hc2_pix,CV_8UC3, c_chapa);
        cv::Mat img_int=Mat::ones(lc2_pix,hc2_pix,CV_8UC1);
/*
        for(int h=-hc; h<hc ; h++ )
        {
            for(int l=-lc; l<lc ; l++)
...
//        cv::waitKey();
        ...
        double hr=long_robot_y*100*0.5;//l_sherpaxl;
        double lr=long_robot_x*100*0.5;//h_sherpaxl;

        //Máscara robot
        cv::Mat img_r=Mat::ones(2*lr+1, 2*hr+1, CV_8UC1);
        ...
        ///Parametros herramienta
         ...
         //Máscara de la herr
         cv::Mat img_h=Mat::zeros(2*abs(xd)+1, 2*abs(yd)+1, CV_8UC1);
                 //img_h.at<uchar>(abs(yd)+1-yd, abs(xd)+1+xd)=1; //?
                 img_h.at<uchar>(1,1)=1; //double long_act_x, double long_act_y
...
void IROBOT_ir_Storage_Service_Class::computeDefectsDecomposition()
{
      ...
      ROS_INFO("Numero defectos: %d", poly_vector.size());
      ...
      for(int i=0; i<poly_vector.size(); i++)
      {
         ...
         ROS_INFO("Numero working H zones: %d", this->working_Hzones_vector.size());
         //0! ¿Por qué?
...
void IROBOT_ir_Storage_Service_Class::computeInspectionZoneFCPP(int id_zona)
{
   ...
   geometry_msgs::Point ref_p;
                        ref_p.x=this->longa_x - this->longr_x; ref_p.y=this->longa_y - this->longr_y; //añadí lo de longr. SARA
 ...
 //  translatePoints(path_result,transf_tras_x,transf_tras_y); //LO COMENTÉ YOO
   ...
   for(auto & elem: path_result)
   {
    ...
    paths_Czones_vector.at(id_vector).header.frame_id="inspection_frame"; //"world"


    //SARA----------
    for (int i=0; i<paths_Czones_vector.size(); i++)
    {
        for (int j=0; j<paths_Czones_vector[i].poses.size(); j++)
        {
            if (round(paths_Czones_vector[i].poses[j].position.x) == round(this->longa_x) )
                paths_Czones_vector[i].poses[j].position.x = this->longa_x - this->longr_x/2;

            else if (round(paths_Czones_vector[i].poses[j].position.x) == 0 )
                paths_Czones_vector[i].poses[j].position.x = this->longr_x/2;

            if (round(paths_Czones_vector[i].poses[j].position.y) == round(this->longa_y) )
                paths_Czones_vector[i].poses[j].position.y = this->longa_y - this->longr_y/2;

            else if (round(paths_Czones_vector[i].poses[j].position.y) == 0 )
                paths_Czones_vector[i].poses[j].position.y = this->longr_y/2;
...
//                                -1*(herr_respecto_orig.position.y-base_respecto_orig.position.y));
   ...
   paths_Hzones_vector.at(id_vector).header.frame_id="sheet_inspection_frame";
      ...
      for(auto & elem: defectos_vec.at(id_defect).defecto_divpor_zonas)
      {
          ...
          if(elem.zona==zona_i)
          {
              for( auto & elemt: elem.trap_vec)
              {
                  ...
                  TrapPathPlanning path(elemt.ros_poly,0.05, type_of_path); //0.15?¿?¿?¿?
                  std::vector<geometry_msgs::Point> path_result;
                ...
                //  trap_toolfcpp_path.header.frame_id="origin_of_the_universe";
                  trap_toolfcpp_path.header.frame_id="sheet_inspection_frame";
                ...
                //  trap_fcpp_path.header.frame_id="origin_of_the_universe";
                  trap_fcpp_path.header.frame_id="sheet_inspection_frame";
...
void IROBOT_ir_Storage_Service_Class::updateVizMsgs()
{
   ...
   // std::string reference_frame_string="origin_of_the_universe";//"summit_xl_a_base_link";//"chapa_orig"; ////Darlo en el constructor y guardarlo como atributo????
    std::string reference_frame_string="sheet_inspection_frame";
    std::string robot_frame_string="summit_xl_base_link";
