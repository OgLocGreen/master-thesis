...
public:
  ...
  void executeCB(const irobot_fcpp_simul::ExecutePathGoalConstPtr &goal)
  {
   ...
   // std::string tf_robot_name="summit_xl_odom_estimated";
  //  std::string tf_reference_frame_name="world";
    ...
    // start executing the action
      ...
      else
      {
         ...
         //Calcular path planning desde el punto más cercano
          ...
          for(int i_path=0; i_path<goal->path.size(); i_path++)
          {
              for(int i_pose=0; i_pose<goal->path.at(i_path).poses.size(); i_pose++)
                  ROS_INFO("%d,%d: X: %f, Y: %f", i_path,i_pose,goal->path[i_path].poses[i_pose].position.x,goal->path[i_path].poses[i_pose].position.y);
          }
         ...
         for(int i_pose=0; i_pose<goal->path.at(i_path).poses.size(); i_pose++)
         {
             ...
             coord_goal.vel=goal->robot_vel;//0.15; //Estaba 0.3
             coord_goal.frame=tf_reference_frame_name;

             ROS_INFO("Enviando goal moverobotcoord");
             ROS_INFO("Enviando goal moverobotcoord: X: %f, Y: %f, frame: %s",coord_goal.movement.x, coord_goal.movement.y, coord_goal.frame.c_str());
