    ...
    // start executing the action
      ...
      else
      {
        ///1.-Obtener la estructura de defectos del servicio
          ...
          ROS_INFO("-----> Inicio de llamada al servicio de FCPP");
        ...
        ///
          ...
          irobot_fcpp_simul::IROBOT_ir_data srv_in;


          //En funcion de peticion accion
          if (goal->operation_type=='c')
            srv_in.request.get_working_zonesc=true;
          else if (goal->operation_type=='h')
            srv_in.request.get_working_zonesh=true;
          ...
          std::vector<geometry_msgs::Polygon> working_zones_vec;
                                             ...
                                             if (goal->operation_type=='h')
                                             {
                                              working_zones_vec.push_back(srv_in.response.zoneh0);
                                              working_zones_vec.push_back(srv_in.response.zoneh90);
                                              working_zones_vec.push_back(srv_in.response.zoneh180);
                                              working_zones_vec.push_back(srv_in.response.zoneh270);
                                             }
                                              if (goal->operation_type=='c')
                                              {
                                              working_zones_vec.push_back(srv_in.response.zonec0);
                                              working_zones_vec.push_back(srv_in.response.zonec90);
                                              working_zones_vec.push_back(srv_in.response.zonec180);
                                              working_zones_vec.push_back(srv_in.response.zonec270);
                                              }
        ...
        ///----------------------REPETIR PARA CADA ZONA---------------
                 ...
                 ///5.- Cubrir zona
                             ...
                             //PATH STEP
                             double path_step=1; //Debe depender del ancho de la imagen o de la herramienta

                             TrapPathPlanning path(working_zones_vec.at(goal->rectangle_id),path_step, type_of_path);
                             ...
                             ////Se puede sustituir por la info del servicio !!!!!
...
//                             }
                 ...
                 result_.result.path=path_result;


         ///-------------------------------------------------------------------
      }
}
  void executeCB_old(const irobot_fcpp_simul::ExecuteRectangleFCPPGoalConstPtr &goal)
  {
    // helper variables
    ros::Rate r(1);
    bool success = true;
    ...
    // start executing the action
      ...
      // this sleep is not necessary, the sequence is computed at 1 Hz for demonstration purposes
        ...
        if(success)
        {
            ...
            ROS_INFO(" Fin de la llamada al servicio de FCPP <-------");
...
int main(int argc, char **argv)
{
    ...
    RectangleFCPPAction FCPPexecution("irobot_ir_executerectfcpp");
