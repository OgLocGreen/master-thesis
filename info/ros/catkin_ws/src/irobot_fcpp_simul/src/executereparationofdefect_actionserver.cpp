...
#include <irobot_fcpp_simul/ExecuteDefectsRepairAction.h>
...
class ExecuteDefectReparationAction
{
protected:

  ros::NodeHandle nh_;
  actionlib::SimpleActionServer<irobot_fcpp_simul::ExecuteDefectsRepairAction> as_; // NodeHandle instance must be created before this line. Otherwise strange error occurs.
  std::string action_name_;

  // create messages that are used to published feedback/result
  //action_server_package::BaseRotationFeedback feedback_;
  irobot_fcpp_simul::ExecuteDefectsRepairResult result_;

public:

  ExecuteDefectReparationAction(std::string name) :
    as_(nh_, name, boost::bind(&ExecuteDefectReparationAction::executeCB, this, _1), false),
    action_name_(name)
  {
    as_.start();
  }

  ~ExecuteDefectReparationAction(void)
  {
  }

  void executeCB(const irobot_fcpp_simul::ExecuteDefectsRepairGoalConstPtr &goal)
  {
    // helper variables
    ros::Rate r(1);
    bool success = true;
...
//                                              working_zones_vec.push_back(srv_in.response.zone1);
//                                              working_zones_vec.push_back(srv_in.response.zone2);
//                                              working_zones_vec.push_back(srv_in.response.zone3);
...
//                                              working_zones_vec.push_back(srv_in.response.zone9);
        ...
        for(int zona_i=0; zona_i<6; zona_i++)
        {

            bool hayDefectoEnZona=false;
            for(auto & elem: estructura_defectos.at(goal->defecto_id).defecto_divpor_zonas)
            {
                if(elem.zona==zona_i)
                {
                    hayDefectoEnZona=true;
                }
            }

            if(!hayDefectoEnZona)
            {
                ROS_INFO("No hay defectos en la zona %d", zona_i);
                continue;
            }
            else
            {
                ROS_INFO("Comienza reparacion de la zona %d", zona_i);
      ...
      // this sleep is not necessary, the sequence is computed at 1 Hz for demonstration purposes
        ...
        if(success)
        {
            ...
            result_.reparado=true; //Quizas necesaripo algo más complejo que un bool
...
{
    ros::init(argc, argv, "executereparationofdefect_actionserver");
    ros::NodeHandle nh;

    ExecuteDefectReparationAction defectreparation("defectreparation");
    ROS_INFO("ExecuteDefectReparation action server is ready!");
