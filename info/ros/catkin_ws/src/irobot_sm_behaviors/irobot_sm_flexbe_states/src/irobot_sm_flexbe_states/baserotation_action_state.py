...
from action_server_package.msg import BaseRotationAction, BaseRotationGoal


class BaseRotationActionState(EventState):
	'''
	State that activate the action of doing a full rotation of the robot base.


	
	># n_degrees 	double 	Number of degrees to rotate.
	...
	def __init__(self):
		...
		super(BaseRotationActionState, self).__init__(outcomes = ['done','failed'],
												 	  input_keys = ['n_degrees'])
		...
		self._topic = 'baserotation' #Aqui mismo topic que en el server
		self._client = ProxyActionClient({self._topic: BaseRotationAction}) # pass required clients as dict (topic: type)
	...
	def on_enter(self, userdata):
		...
		# Create the goal.
		goal = BaseRotationGoal()
		goal.number_of_degrees = n_degrees
