...
class ActivateIRVizualizationState(EventState):
    '''
    State to active the Rviz messages in IR srv


    <= done                 Rviz activated
    <= failed               Rviz not activated

    '''

    def __init__(self):
        # Declare outcomes, input_keys, and output_keys by calling the super constructor with the corresponding arguments.
        super(ActivateIRVizualizationState, self).__init__(outcomes = ['done', 'failed'])

        self.exito_inicial=0

    def execute(self, userdata):
        # This method is called periodically while the state is active.
        print "Activando visualizacion Rviz-------->"
