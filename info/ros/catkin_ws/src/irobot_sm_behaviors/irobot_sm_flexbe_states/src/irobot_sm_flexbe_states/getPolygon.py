...
from irobot_inspection_pckg.msg import getPolygonAction, getPolygonGoal
...
class GetPolygonState(EventState):
	'''
	...
	def __init__(self):
		...
		super(GetPolygonState, self).__init__(outcomes = ['done','failed'], input_keys = ['folder_path','sheet_size_x', 'sheet_size_y'])


		
		self._topic = 'getpolygon' #Mismo nombre que en el server
		...
		self._client = ProxyActionClient({self._topic: getPolygonAction})
	...
	def on_enter(self, userdata):
		...
		# Create the goal.
		goal = getPolygonGoal()
		goal.path.data = userdata.folder_path
		goal.sheet_size_x = userdata.sheet_size_x
		goal.sheet_size_y = userdata.sheet_size_y
		...
		try:
			...
			Logger.logwarn('Failed to send the getPolygon command:\n%s' % str(e))
			self._error = True
