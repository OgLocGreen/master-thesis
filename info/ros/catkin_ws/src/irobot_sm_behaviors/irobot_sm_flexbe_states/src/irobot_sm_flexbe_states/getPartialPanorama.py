...
from irobot_inspection_pckg.msg import getPartialPanoramaAction, getPartialPanoramaGoal
...
class GetPartialPanoramaState(EventState):
	'''
	...
	def __init__(self):
		...
		super(GetPartialPanoramaState, self).__init__(outcomes = ['done','failed'], input_keys = ['folder_path', 'direction','folder_pano_path'])


		
		self._topic = 'getpartialpanorama' #Mismo nombre que en el server
		...
		self._client = ProxyActionClient({self._topic: getPartialPanoramaAction})
	...
	def on_enter(self, userdata):
		...
		try:
			...
			Logger.logwarn('Failed to send the getPartialPanorama command:\n%s' % str(e))
			self._error = True
