...
from irobot_dimensions_pckg.msg import obtainDimensionAction, obtainDimensionGoal
...
class ObtainDimensionState(EventState):
	'''
	...
	def __init__(self):
		...
		super(ObtainDimensionState, self).__init__(outcomes = ['done','failed'], input_keys = ['n_imgs1', 'folder_path1', 'n_imgs2', 'folder_path2'], output_keys = ['largo', 'ancho'])


		
		self._topic = 'obtaindimension' #Mismo nombre que en el server
		...
		self._client = ProxyActionClient({self._topic: obtainDimensionAction})
	...
	def on_enter(self, userdata):
		...
		# Create the goal.
		goal = obtainDimensionGoal()
		goal.n_imgs1 = userdata.n_imgs1
		goal.n_imgs2 = userdata.n_imgs2
		goal.folder_path1.data = userdata.folder_path1
		goal.folder_path2.data = userdata.folder_path2
		...
		try:
			...
			Logger.logwarn('Failed to send the obtaindimension command:\n%s' % str(e))
			self._error = True
